...
test("server-renders the expanded mobile study coach", async () => {
  ...
  // 주된 행동은 어디로 가는지 이름을 말해야 한다. "오늘 여기서 시작하기"처럼
  // 목적지를 감추면 바로 아래 과목 카드 셋과 같은 무게로 보인다 — 실제로 그
  // 버튼과 과목 카드는 같은 동작이었다.
  // React가 변수와 문자열 사이에 <!-- -->를 넣는다. 위 단언들과 같은 처리다.
  assert.match(
    html,
    /(국어|영어|수학)(?:<!-- -->)? 한 칸 시작하기/,
    "주된 행동이 어느 과목인지 말해야 한다",
  );
  // 문구만 확인하면 버튼이 둘로 늘어난 것을 못 잡는다. 하나여야 한다는 것이
  // 이 단언의 원래 의도이므로 개수를 센다.
  const primaryCtaCount = (html.match(/한 칸 시작하기/g) ?? []).length;
  assert.equal(primaryCtaCount, 1, "첫 화면의 주된 행동은 하나여야 한다");
